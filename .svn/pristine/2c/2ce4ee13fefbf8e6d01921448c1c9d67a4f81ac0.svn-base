...
function loadPoint() {
    $("#profit-data").empty();
    var _data = "{ action:'queryPoint',page:'" + page + "',type:'" + type + "',beginTime:'" + beginTime + "',endTime:'" + endTime + "'}";
    $.ajax({
        type: "POST",
...
function loadPointShop() {
    ...
    $.ajax({
        ...
        success: function (resu) {
            ...
            for (var i = 0; i < data.length - 2; i++) {
                var obj = data[i];
                var time = Date2MMDDYYYY(obj.addTime);
                var remark = obj.remark;
                if (remark == null) {
                    remark = "";
                }
                var tr;
                if (obj.point > 0) {
                    tr = '<tr><td>' + time + '</td> <td>+' + obj.point + '</td><td><i class="blue">Order ID：' + obj.orderNo + '</i></td><td>' + remark + '</td></tr>';
                } else {
                    tr = '<tr><td>' + time + '</td> <td>' + obj.point + '</td><td><i class="blue">Order ID：' + obj.orderNo + '</i></td><td>' + remark + '</td></tr>';
