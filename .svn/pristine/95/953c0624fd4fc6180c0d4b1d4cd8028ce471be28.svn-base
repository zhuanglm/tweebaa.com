<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="proDownMgr.aspx.cs" Inherits="TweebaaWebApp.Mgr.proManager.proDownMgr" %>
...
<fieldset>
        ...
        <table class="tbtable" style="width:100%;">
            ...
            <tr>
                ...
                <td class="title">Evaluation Detail：<asp:HiddenField ID="hidState" runat="server" /></td>
                <td>
   ...
   <div style=" margin:2px;">
        ...
        <script type="text/javascript">
            $(function () {
                ...
                $("#ddlTypeOne").combobox({
                    valueField: 'value',
                    textField: 'text',
                    data: eval(TweebaaWebApp.Mgr.proManager.proDownMgr.GetFirstCate().value),
                    onSelect: function (e) {
                        var selectedId = e.value;
                        $("#ddlTypeTwo").combobox("clear");
                        $("#ddlTypeThree").combobox("clear");
                        if (selectedId != "-1") {
                            var res = TweebaaWebApp.Mgr.proManager.proDownMgr.GetNextCate(selectedId).value;
                            if (res != "fail") {
                ...
                $("#ddlState").combobox({
                    valueField: 'value',
                    textField: 'text',
                    data: eval("[{value:'-1',text:'--ALL--'},{value:'9',text:'Already Removed'}]"),
                    onSelect: function (e) {
        ...
        <script type="text/javascript">
            var dataGridUrl = 'proPublicJudgeMgr.ashx?action=downdatagrid';
            ...
            $(function () {
                var pager = $('#dg').datagrid({
                    ...
                    columns: [[
                        ...
                        //{ field: 'estimateprice', title: 'Estimated Price', width: 80 },
                        //{ field: 'saleprice', title: 'Limited Time Price', width: 80 },
                        { field: 'count', title: 'Sales', width: 80, aligh:'right'},
                        { field: 'saletotal', title: 'Total Sales Amount', width: 80, aligh: 'right',
                        ...
                        //{ field: 'count1', title: 'Number of Reject', width: 80 },
                        { field: 'wnstat', title: 'Evaluation Status', width: 80 },
                        { field: 'username', title: 'Submitter', width: 100 },
                        { field: 'addtime', title: 'Submission Time', width: 100 },
                        { field: 'stateremark', title: 'Evaluation Reason', width: 100 },
                        {
                            field: '_edit', title: 'Operation', width: 350, formatter: function (value, row) {
                                return "<a href='javascript:void(0)' onclick=_view(this) dbkey='" + row.prdguid + "'>Check</a>&nbsp;&nbsp;<a href='javascript:void(0)' dbkey='" + row.prdguid + "' dbproname='" + row.prdname + "' onclick=DownSingle(this)>List Again</a>&nbsp;&nbsp;<a href='javascript:void(0)' dbkey='" + row.prdguid + "' onclick=proEdit(this)>Edit</a>"
                            }
                        }
                    ]]
            ...
            function noPassSave() {
                ...
                if (proid == null || reason == null || proid == "" || $.trim(reason) == "") {
                    ...
                    if ($("#nopassReason").attr('tip') == 'all') {
                        res = TweebaaWebApp.Mgr.proManager.proDownMgr.UpAll(proid, reason).value; //批量上架
                    } else {
                        res = TweebaaWebApp.Mgr.proManager.proDownMgr.UpSingle(proid, reason).value; //间个上架
                    }
