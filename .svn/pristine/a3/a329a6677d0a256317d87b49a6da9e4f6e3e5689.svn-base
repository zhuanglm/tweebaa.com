<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="proPublicJudgeMgr.aspx.cs" 
 ValidateRequest="false" enableEventValidation="false" Inherits="TweebaaWebApp.Mgr.proManager.proPublicJudgeMgr" %>
        ...
        <script type="text/javascript">
            $(function () {
                ...
                $("#ddlTypeOne").combobox({
                    valueField: 'value',
                    textField: 'text',
                    data: eval(TweebaaWebApp.Mgr.proManager.proPublicJudgeMgr.GetFirstCate().value),
                    onSelect: function (e) {
                        var selectedId = e.value;
                        $("#ddlTypeTwo").combobox("clear");
                        $("#ddlTypeThree").combobox("clear");
                        if (selectedId != "-1") {
                            var res = TweebaaWebApp.Mgr.proManager.proPublicJudgeMgr.GetNextCate(selectedId).value;
                            if (res != "fail") {
        ...
        <script type="text/javascript">
            ...
            function passAll() {
                if (confirm("Confirm to batch approve without any verification?？")) {
                    ...
                    if (checkedItems.length == 0) {
                        ...
                        if (proids.length > 0) {
                            var res = TweebaaWebApp.Mgr.proManager.proPublicJudgeMgr.PassAll(proids).value;
                            if (res == "success") {
            ...
            function passSingle() {
                ...
                var adminid = $("#hidMgrAdminid").val();
                    if (proid.length > 0) {
                        var res = TweebaaWebApp.Mgr.proManager.proPublicJudgeMgr.PassSingle(proid,reasonid,exreason,adminid).value;
                        if (res == "success") {
                            alert("Approve Success！");
                            $('#winView').window('close');
                            _search();
            ...
            function OpenReason(obj) {
                ...
                if (isNaN(up) || isNaN(down)) {
                    alert("Participants cannot reach" + count + "，cannot approve");
                    return;
                }
                if ((up + down) < count) {
                    alert("Participants cannot reach"+count+"，cannot approve");
                ...
                if ((up + down) <= 0) {
                    alert("Supports cannot reach"+bili+"，cannot approve");
                    return;
                }
