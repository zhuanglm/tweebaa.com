...
namespace Twee.Comm
{
    ...
    public class XmlHelper
    {
        ...
        /// <returns>成功返回true,失败返回false</returns>
        public static bool CreateXmlNodeByXPath<K, V>(string xmlFileName, string xpath, string xmlNodeName, string innerText, Dictionary<K, V> dt)
        {
            bool isSuccess = false;
            XmlDocument xmlDoc = new XmlDocument();
            try
            {
                ...
                if (xmlNode != null)
                {
                    ...
                    foreach (KeyValuePair<K, V> c in dt)
                    {

                        if (!string.IsNullOrEmpty(c.Key.ToString()) && !string.IsNullOrEmpty(c.Value.ToString()))
                        {
                            XmlAttribute xmlAttribute = xmlDoc.CreateAttribute(c.Key.ToString());
                            xmlAttribute.Value = c.Value.ToString();
