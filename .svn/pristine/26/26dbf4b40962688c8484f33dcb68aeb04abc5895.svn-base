    ...
    $.ajax({
        ...
        success: function (resu) {
            ...
            for (var i = 0; i < obj.length; i++) {
                ...
                var onErrorImg = picPathCollage2 + prd.CollageDesign_ThumbnailFileName;
                ...
                */
                ...
                html = '<tr><td><a href="' + urlPage + '" target="_blank" class="imglink fl"><img style="max-width:320px;" class="full-width img-responsive" onerror="this.src=\''+onErrorImg+'\'" src="' + picPathCollage1 + prd.CollageDesign_ThumbnailFileName + '" alt="" /></a>"';
                html += '<div class="pro-title fl" style="margin-left:20px;"><a href="' + urlPage + '" target="_blank">' + prd.CollageDesing_Title + '</a><p>	Favorite time： <br/>' + prd.edttime.replace("T", " ").substring(0, 16) + '</p>	</div></td>"';
                html += '<td><div class="btn-group"><button class="btn rounded btn-default " type="button" onclick="ShareCollage(' + prdid + ',' + prdname + ',' + prdimg + ',' + urlPage + ',' + prdDesc + ')" ><i class ="fa fa-share-alt"</i></button></td>';
                html += '<td><button class="btn rounded btn-default" type="button" onclick=" DeleteCollageFavorite(' + guid + ')"><i class="fa  fa-trash"></i></button></div></td></tr>';
...
//删除我的收藏
function DeleteCollageFavorite(prdid) {
