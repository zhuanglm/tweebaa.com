...
namespace Twee.Mgr
{
    public class PublistGradeParamMgr
    {
        ILog log = log4net.LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
        PublistGradeParamDataMgr mgr = new PublistGradeParamDataMgr();
        ...
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public Twee.Model.Publishgradeparam GetModelByCache()
        {
            //该表无主键信息，请自定义主键/条件字段
            string CacheKey = "wn_publishgradeparamModel-";
            ...
            return (Twee.Model.Publishgradeparam)objModel;
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<Twee.Model.Publishgradeparam> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Twee.Model.Publishgradeparam> DataTableToList(DataTable dt)
        {
            List<Twee.Model.Publishgradeparam> modelList = new List<Twee.Model.Publishgradeparam>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                Twee.Model.Publishgradeparam model;
        ...
        public bool MgeAdd(int grade, int integral, int reviewreward, int presaleward, int reviewsupportmin, int reviewsupportmax, int presalesupportmin, int presalesupportmax, int commissionratio)
        {
            try
            {
                return mgr.MgeAdd(grade, integral, reviewreward, presaleward, reviewsupportmin, reviewsupportmax, presalesupportmin, presalesupportmax, commissionratio);
            }
            catch (Exception ex)
            {
                log.Error("error", ex);
        ...
        public bool MgeUpdate(Guid guid, int grade, int integral, int reviewreward, int presaleward, int reviewsupportmin, int reviewsupportmax, int presalesupportmin, int presalesupportmax, int commissionratio)
        {
            try
            {
                return mgr.MgeUpdate(guid, grade, integral, reviewreward, presaleward, reviewsupportmin, reviewsupportmax, presalesupportmin, presalesupportmax, commissionratio);
            }
            catch (Exception ex)
            {
                log.Error("error", ex);
