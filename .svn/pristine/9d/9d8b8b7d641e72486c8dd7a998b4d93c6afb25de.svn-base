<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="proTweeJudgeMgr.aspx.cs" Inherits="TweebaaWebApp2.Mgr.proManager.proTweeJudgeMgr" %>
...
<body style="padding:2px;">
        ...
        <table class="tbtable" style="width:100%;">
            ...
            <tr>
                <td colspan="8">
                    ...
                    <a id="A1" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-save'" onclick="passAll()" style="display:none;">Batch approval for Test-Sale Products</a>
   ...
   <table id="dg" title="Product Evaluation List" style="width:100%;height:auto"></table>
        ...
        <div id="preSaleDiv" title="Edit Test-Sale Price, Target and Time">
                   <table id="Table1" class="tbtable" style="width:300px;">
                       <tr class="head">
                           <td class="title" style=" width:100px;">Test-Sale Price：</td>
                           <td><input type="text"  id="txtPrePrice" /></td>
                       </tr>
                       <tr class="head">
                           <td class="title" style=" width:100px;">Test-Sale Target：</td>
        ...
        <script type="text/javascript">
            $(function () {
                ...
                $("#ddlTypeOne").combobox({
                    valueField: 'value',
                    textField: 'text',
                    data: eval(TweebaaWebApp2.Mgr.proManager.proTweeJudgeMgr.GetFirstCate().value),
                    onSelect: function (e) {
                        var selectedId = e.value;
                        $("#ddlTypeTwo").combobox("clear");
                        $("#ddlTypeThree").combobox("clear");
                        if (selectedId != "-1") {
                            var res = TweebaaWebApp2.Mgr.proManager.proTweeJudgeMgr.GetNextCate(selectedId).value;
                            if (res != "fail") {
                ...
                $("#ddlState").combobox({
                    valueField: 'value',
                    textField: 'text',
                    data: eval("[{value:'-1',text:'--ALL--'},{value:'4',text:'Advance to Test-Sale'},{value:'5',text:'Evaluation Fail'}]"),
                    onSelect: function (e) {
        ...
        <script type="text/javascript">
                ...
                var pager = $('#dg').datagrid({
                    ...
                    //{ field: 'estimateprice', title: '预估价格', width: 80 },
                        {field: 'saleprice', title: 'Test-Sale Price', width: 80, align: 'right' },
                         { field: 'presaleforward', title: 'Test-Sale Target', width: 80, align: 'right' },
                          { field: 'presaleendtime', title: 'Tesat-Sale End Time', width: 80,
            ...
            function passAll() {
                if (confirm("Confirm to approve selected products？")) {
                    ...
                    if (checkedItems.length == 0) {
                        ...
                        if (proids.length > 0) {
                            var res = TweebaaWebApp2.Mgr.proManager.proTweeJudgeMgr.PassAll(proids).value;
                            if (res == "success") {
                                alert("Approve Success！");
                                _search();
                            }
                            else if (res == "noInventory") {
                                alert("Product does not have an approved supplier or Test-Sale target, cannot advance to Test-Sale!");
                            }
                            else if (res == "fail") {
                                alert("Approve Fail！");
                            }
                               ...
                               alert("{Partial operation success}--Product does not have an approved supplier or Test-Sale target, so cannot advance to Test-Sale：" + alterProName);
                                _search();
            ...
            function passSingle(obj) {
                if (confirm("Confirm to approve selected products？")) {
                    ...
                    if (proid.length > 0) {
                        var res = TweebaaWebApp2.Mgr.proManager.proTweeJudgeMgr.PassSingle(proid).value;
                        if (res == "success") {
                            alert("Approve Success！");
                            _search();
                        }
            ...
            function noPassSave() {
                ...
                if (proid == null || reason == null || proid == "" || $.trim(reason) == "") {
                    ...
                    var res = TweebaaWebApp2.Mgr.proManager.proTweeJudgeMgr.NoPass(proid, reason).value;
                    if (res == "success") {
                        alert("Approve Success");
                        $('#winView').window('close');
                        _search();
            ...
            function FowardSave() {
                ...
                if (proids.length > 0) {
                    ...
                    var res = TweebaaWebApp2.Mgr.proManager.proTweeJudgeMgr.SavePreSale(proids,prePrice, order, time).value;
                    if (res == "success") {
                        alert("Edit Success");
                        $('#preSaleDiv').window('close');
                        _search();
