    ...
    while (true) {
        ...
        if (isVisible) { // deleted categories are hidden)
            ...
            if (prdCate1 == "" || prdCate1 == null || prdCate1 == "-1") {
                AlertEx("Please select the product category！");
                $(prdType1Name).focus();
                return false;
            }
            ...
            if (prdCate3List.indexOf(prdCate3) != -1) {
                AlertEx("Category is duplicated！");
                $(prdType3Name).focus();
                return false;
            }
...
function doAddPrd(action, type) {
    ...
    if (prdName == "") {
        Loading(false);
        AlertEx("Please input the name of the product！");
        $("#pro-name").focus();
        return inputError;
    }
    ...
    */
    ...
    if (!CheckAndGetPrdCate()) return inputError;
    ...
    if (prdTag != null && prdTag != "") {
        ...
        if (arrTag.length > 10) {
            Loading(false);
            AlertEx("Max 10 tags are allowed!");
            $("#txtTag").focus();
            return inputError;
        }
        ...
        for (var i = 0; i < arrTag.length; i++) {
            if (arrTag[i].length <= 0) {
                Loading(false);
                AlertEx("Please enter valid tag name!");
                $("#txtTag").focus();
                return inputError;
            }
        }
    ...
    var prdAdvantage = $("#txtBriefDesc").val(); //卖点特色
    if (prdAdvantage == "") {
        Loading(false);
        AlertEx("Please describe the products brief selling point characteristics！");
        $("#txtBriefDesc").focus();
        return inputError;
    }
    ...
    if (prdContent == "" || prdContent == null) {
        Loading(false);
        AlertEx("Please describe the products detail selling point characteristics！");
        $("#pro-des").focus();
        editor.focus();
        return inputError;
    }
    ...
    if (prdPrice == "" || prdPrice == null) {
        Loading(false);
        AlertEx("Please input the suggested selling price！");
        $("#txtPrice").focus();
        return inputError;
    }
    ...
    // minimum is $0.01
    if (parseFloat(prdPrice) < 0.01) {
        Loading(false);
        AlertEx("Minumum suggested selling price is $0.01\nPlease enter a valid value!");
        $("#txtPrice").focus();
        return inputError;
    }
    ...
    if (supplyPrice == "" || supplyPrice == null) {
        Loading(false);
        AlertEx("Please input the supply price！");
    ...
    if (parseFloat(supplyPrice) < 0.01) {
        Loading(false);
        AlertEx("Minumum supply price is $0.01\nPlease enter a valid value!");
        $("#txtSupplyPrice").focus();
        return inputError;
    }

    // supply price cannot over selling price
    if (parseFloat(supplyPrice) > parseFloat(prdPrice)) {
        Loading(false);
        AlertEx("Your Supply Price should not be higher than your Suggested Selling Price. Please try again.");
        $("#txtPrice").focus();
        return inputError;
    }
    ...
    if (moq == "" || moq == null) {
        Loading(false);
        AlertEx("Please prove the lowest minimum order quantity!");
        $("#txtMoq").focus();
        return inputError;
    }
    ...
    if (isSupplier == 0) {
        ...
        if (supplierName == "" || supplierName == null) {
            Loading(false);
            AlertEx("Please provide the supplier name when you are not the product supplier!");
        ...
        if (iError == 4) {
            Loading(false);
            AlertEx("Please provide AT LEAST one of supplier website, phone number, email and address!");
        ...
        // above reg is too complex use a simple reg instead. 
        var regURL = new RegExp('^(http|https):\/\/[^ "]+$', 'i');
        
        if (supplierWebsite != "" && supplierWebsite != null) {
            if (!regURL.test(supplierWebsite)) {
                Loading(false);
                AlertEx("Please input a valid supplier website!");
                $("#txtSupplierWebsite").focus();
                return inputError;
            }
        }
        ...
        /*
        //modify by Long as other country
        var regPhone = /^\(\d{3}\) ?\d{3}( |-)?\d{4}|^\d{3}( |-)?\d{3}( |-)?\d{4}$/;
        if (supplierPhone != "" && supplierPhone != null) {
        if (!regPhone.test(supplierPhone)) {
        AlertEx("Please input a valid supplier phone number!");
        ...
        */
        ...
        /*
        if (supplierEmail.indexOf("@") == -1 || supplierEmail.indexOf(".") == -1) {
            AlertEx("Please input a valid supplier email!");
    ...
    if (piclist == "") {
        Loading(false);
        AlertEx("Please upload at least one image of the product!");
        return inputError;
    }
    if (prdVideo != "") {
...
function addPrd(action, type) {
    ...
    if (retDoAddPrd == inputError || retDoAddPrd == ajaxError) {
        ...
        AlertEx("Logged in to release products！");
        return;
    }
...
function addPrd2(action, type) {
    ...
    */
    ...
    var prdPrice = $("#txtPrice").val(); //建议零售价
    if (prdPrice == "" || prdPrice == null) {
       // AlertEx("Please input the suggested retail price！");
       // return;
    }
    var prdPic = "";   
    var strVideo = $("#hidvideo").val();
    ...
    if (piclist == "") {
        //AlertEx("Please upload at least a product thumbnail！");
        //return;
    }
    if (prdVideo != "") {
        videoTemp = prdVideo;
    ...
    $.ajax({
        ...
        success: function (resu) {  
            ...
            if (resu.length > 5) {
                ...
                AlertEx("Your work has been saved. To access, just log in to your account and you'll find it in the Suggestions section.");
                return;
            }
            else if (resu == "error") {
                AlertEx("Save failed！");
    ...
    $.ajax({
        ...
        success: function (msg) {
            if (msg == "success") {
                ...
                //AlertEx("Submitted successfully!");
                Loading(false);
                AlertEx("Failed  to suggest");
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            Loading(false);
...
function backEdit() {
    ...
    window.location.href = "SubmitForm.aspx?action=edit&id=" + prdID;
}
    ...
    if (action == "edit" && prdID != null) {
        $.ajax({
            ...
            success: function (resu) {
                ...
                $("#txtBriefDesc").val(baseInfo.txtjj); //卖点特色
                limitLenth($("#txtBriefDesc"), 450, "ltesetip");
                if (baseInfo.txtjj != null && baseInfo.txtjj != "") {
                    $("#txtBriefDesc").siblings('.tishi').hide();
                    $("#txtBriefDesc").focus();
                    $("#pro-name").focus();
                }
                ...
                $("#supplierSection").css("display", supplierDisp); //Add by Long 2016/02/03
        ...
        // check login id not login then promote user to login and redirect to login page
        // after user login, back to the suggest page
        if (!CheckUserLogin()) {
            AlertEx("Please login");
...
function AddAnotherCategory() {
    ...
    var htmlPrdCate = '<tr style=" margin-bottom:20px;" id="' + trPrdCateName + '">' +
                                     ...
                                     '   </div>' +
                                    '</td></tr>';*/
    var htmlPrdCate = '<div class="row" id="' + trPrdCateName + '">';
            htmlPrdCate = htmlPrdCate+'<section class="col col-3">';
            htmlPrdCate = htmlPrdCate+'<label class="select">';
            htmlPrdCate = htmlPrdCate+'<select name="" class="tag_select" id="prdType' + idx + '1" >';
            htmlPrdCate = htmlPrdCate+' </select>';
            htmlPrdCate = htmlPrdCate+'<i></i>';
            htmlPrdCate = htmlPrdCate+'</label>';
            htmlPrdCate = htmlPrdCate+'</section>';
            htmlPrdCate = htmlPrdCate+'<section class="col col-3">';
            htmlPrdCate = htmlPrdCate+'<label class="select">';
            htmlPrdCate = htmlPrdCate+'<select name="" class="tag_select" id="prdType' + idx + '2">';
            htmlPrdCate = htmlPrdCate+'</select>';
            htmlPrdCate = htmlPrdCate+'<i></i>';
            htmlPrdCate = htmlPrdCate+'</label>';
            htmlPrdCate = htmlPrdCate+'</section>';
            htmlPrdCate = htmlPrdCate+'<section class="col col-3">';
            htmlPrdCate = htmlPrdCate+'<label class="select">';
            htmlPrdCate = htmlPrdCate+' <select name="" class="tag_select" id="prdType' + idx + '3">';
            htmlPrdCate = htmlPrdCate+' </select>';
            htmlPrdCate = htmlPrdCate+' <i></i>';
            htmlPrdCate = htmlPrdCate+'</label>';
            htmlPrdCate = htmlPrdCate + '</section><section class="col col-3"><a rel="trim" onclick="DeletePrdCate(' + idx.toString() + ')" class="btn-u btn-u-red rounded trim">Delete</a></section></div>';
...
function LimitLen(element, len) {
    //alert(len + " " + element.value + " " + element.value.substr(0, len));
