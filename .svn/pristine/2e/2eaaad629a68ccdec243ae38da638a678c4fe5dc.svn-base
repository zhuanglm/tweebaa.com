...
$(document).ready(
   ...
   function loadGrandTotal() {
       ...
       $.ajax({
           type: "Post",
           url: "/AjaxPages/shareAjax.aspx",
           data: "{'action':'queryhomegrandtotal'}",
           async: false,
           success: function (resu) {
               if (resu == "") {
                   return;
       ...
       $("#tblShareList tbody").html("");
        ...
        $.ajax({
            ...
            success: function (resu) {
                ...
                var htmlTotal = '<tr><td colspan=2 width="430" class="bc" style="text-align:center">TOTAL</td>><td width="100" class="bc" style="text-align:center">' + totalProductShareVisitCount + '</td><td class="bc" style="text-align:center">' + totalProductShareSoldQuantity + '</td><td class="bc"></td></tr>';
                $("#tblShareList tbody").append(htmlTotal);
                ...
                while (i < obj.length) {
                    ...
                    for (var j = i; j < obj.length; j++) {
                        if (prdName != obj[j].name) break;
                        //htmlDetail += '<tr id="tr' + j + '" style="display:none; "><div class="details_tb">';
                        htmlDetail += '<tr id="tr' + j + '" style="display:none; background-color:white" class="details_tb">';
                        if (j == i) htmlDetail += '<td rowspan="' + prdRowCount + '" style="background-color:white"></td>';
                        htmlDetail += '<td style="text-align:center;background-color:white">' + obj[j].sharetype + '</td>'
                                 + '<td style="text-align:center;background-color:white">' + obj[j].VisitCountSum + '</td>'
                                 + '<td style="text-align:center;background-color:white">' + obj[j].SoldQuantitySum + '</td>'
                                 + '<td style="width:130px;background-color:white" ><a class="submit simple"  style="color:white;border:none;width:50px;height:25px;" onclick="void(0); CopyToClip(' + copyText + ')"> &nbsp;&nbsp;Copy&nbsp;&nbsp;</a></td>';
                    ...
                    var htmlSubTotal = '<tr class="details_tb"><td style="width:30%; text-align:left" ><a onclick="void(0); DoExpandShareDetail(' + i + ',' + prdRowCount + ')">' + prdName + '</a></td>'
                        + '<td style="text-align:center"><strong>Sub-Total</strong></td>'
                        + '<td style="text-align:center">' + prdClickSum + '</td>'
                        + '<td style="text-align:center">' + prdSoldQtySum + '</td>'
                        + '<td style="width:130px;" >&nbsp;</td></tr>';

                    // display sub-total and details
                    $("#tblShareList tbody").append(htmlSubTotal);
                    $("#tblShareList tbody").append(htmlDetail);
