...
namespace Twee.Mgr
{
    public class SharegradeparamMgr
    {
        ILog log = log4net.LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
        SharegradeparamDataMgr mgr = new SharegradeparamDataMgr();
        ...
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public Twee.Model.Sharegradeparam GetModelByCache()
        {
            //该表无主键信息，请自定义主键/条件字段
            string CacheKey = "wn_sharegradeparamModel-";
            ...
            return (Twee.Model.Sharegradeparam)objModel;
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<Twee.Model.Sharegradeparam> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Twee.Model.Sharegradeparam> DataTableToList(DataTable dt)
        {
            List<Twee.Model.Sharegradeparam> modelList = new List<Twee.Model.Sharegradeparam>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                Twee.Model.Sharegradeparam model;
        ...
        public bool MgeAdd(int grade, int integral, int visitreward, int buyreward, int visitcountmin, int visitcountmax, int commissionratio)
        {
            try
            {
                return mgr.MgeAdd(grade, integral, visitreward, buyreward, visitcountmin, visitcountmax, commissionratio);
            }
            catch (Exception ex)
            {
                log.Error("error", ex);
        ...
        public bool MgeUpdate(Guid guid, int grade, int integral, int visitreward, int buyreward, int visitcountmin, int visitcountmax, int commissionratio)
        {
            try
            {
                return mgr.MgeUpdate(guid, grade, integral, visitreward, buyreward, visitcountmin, visitcountmax, commissionratio);
            }
            catch (Exception ex)
            {
                log.Error("error", ex);
