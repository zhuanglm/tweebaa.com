...
function CalcTotal(countryID, provinceID) {
    ...
    for (var i = 0; i < iTotalProduct; i++) {
        ...
        if (sShipPartnerID != "3") { // 3: drop-shipper
            fShipFee = parseFloat($("#shipfee" + i.toString()).html().replace("$",""));
    ...
    if (countryID == 2) // Canada 
    {
        ...
        // show and hide
        $("#trTax").hide();
        if (fTaxRateGST > 0) $("#trTaxGST").show();
        else $("#trTaxGST").hide();
        if (fTaxRateHST > 0) $("#trTaxHST").show();
        else $("#trTaxHST").hide();
        if (fTaxRateQST > 0) $("#trTaxQST").show();
        else $("#trTaxQST").hide();
    }
    ...
    $("#trShipCost").show(); // show shipping fee after the value is populated
    ...
    $.ajax({
        ...
        success: function (resu) {
            ...
            if (resu == "-1") {
                ...
                for (var i = 0; i < obj.length; i++) {
                    ...
                    if (!isNaN(parseInt(countryID))) {
                        if (cart.ShipPartner_ID == 3) {  // 1: fosdicks 2: EOrder 3:Drop-shipper 
                            ...
                            // do not include price if it is customer free shipping  
                    ...
                    if (shipFeeList != null && shipFeeList != "" && shipFeeList != "0") {
                        ...
                        //alert(shipFeeObj);
                        if (shipFeeObj != null) {
                            // set defaault method as the first method
                            var bFind = false;
                    ...
                    //var html = '<tr><td class="first"><a href="#" class="imglink fr"><img src="' + img + '" alt="" width="55" height="55"/></a></td>';
                    //html += '<td><div class="des-box"><a href="#">' + cart.name + '  </a></div><div style="float:left; padding-left:15px;">Color：</div><div style="float:left;width:20px;height:20px;background-color:' + cart.color + '; "></div><div style="clear:both; text-align:left;padding-left:15px;">Size：' + cart.prorule + '</div></td><td class="pro-zt">' + salseState + '</td><td class="pro-price">' + cart.saleprice + '</td><td class="pro-num">' + cart.quantity + '</td><td class="pro-zhe" style="display:none;">省' + off + '</td><td class="pro-total"><b>$</b><strong>' + money + '</strong></td></tr>';
                    var fltMoney = 0.0;
                    if (money.length > 0) {
                    ...
                    //+ '<td><div class="des-box"><a href="#">' + cart.name + '  </a><br/><span>Color：' + cart.color + '</span>&nbsp&nbsp&nbsp&nbsp<span>Specs：' + cart.prorule + '</span></div></td>'
                ...
                //alert(payMoney); alert(curentLogistic); alert($("#labTax").text());
                //var sumPay = accAdd(payMoney, curentLogistic, $("#labTax").text()).toFixed(2);
...
function AliyPay() {
    ...
    var orderBody = ""; //订单描述
    var orderWebUrl = "http://www.tweebaa.com";
...
function PayPalPay() {  
   // setCookies();
    ...
    var sumUse = Number(pointMoney) + Number(tweebucks);
    if (sumUse > prdMoney) {
    ...
    var txtShipping = $("#labAddress").text() + "," + $("#labCountry").text();  
    var txtOrderExtra = Base64.encode($("#extra_info_table").html());
