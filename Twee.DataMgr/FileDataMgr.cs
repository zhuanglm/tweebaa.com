...
namespace Twee.DataMgr
{
    public class FileDataMgr
    {
		...
		/// <summary>
		/// 删除一条数据
        /// This function deletes all images of a product Commented by Jack Cao 2016-01-19
		/// </summary>
        ...
        public bool DeleteByFileGuid(Guid fileGuid)
        {
            ...
            SqlParameter[] parameters = {
					new SqlParameter("@fileguid", SqlDbType.UniqueIdentifier,16) };
            parameters[0].Value = fileGuid;
        ...
        // add a image from backend
        public bool MgeAdd(Twee.Model.File model, out string sErrMsg)
        {
            ...
            try {
                ...
                strSql.Append("select count(*) from wn_file");
                strSql.Append(" where idx = " + model.idx.ToString());
                strSql.Append("   and prdguid='" + model.prdguid + "'");
                int iCnt = db.DBQueryCount(strSql.ToString());
                if (iCnt > 0)
                {
                    // move all higher index up
                    strSql.Clear();
                    strSql.Append("update wn_File set idx = idx + 1 ");
                    strSql.Append(" where idx >= " + model.idx.ToString());
                    strSql.Append("   and prdguid='" + model.prdguid + "'");
                    iCnt = db.DBExecute(strSql.ToString());
                }
                ...
                if (model.UpLoadBatchNo == null) model.UpLoadBatchNo = -1;
                parameters[10].Value = model.UpLoadBatchNo;
                iCnt = db.DBExecute(strSql.ToString(), parameters);
