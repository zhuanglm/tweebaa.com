...
var FancyProductDesigner = function(elem, opts) {
	...
	//load sidebar html
	$.post(options.templatesDirectory+'ProductDesigner.ashx',
		options.labels,
		function(html){
		...
		//image
			...
			if(colorizable == 'png' || colorizable == 'dataurl') {
				...
				try {
					...
					thisClass.showModal("Image element could not be colorized. Please be sure that the image is hosted under the same domain and protocol,!");
				}
		...
		stage.setDimensions({width:options.width, height: options.stageHeight}).setBackgroundColor(backgroundColor, function() {
			...
			for(var i=0; i<viewsLength;++i) {
				...
				try {
					...
					thisClass.showModal("Error: Please be sure that the images are hosted under the same domain and protocol,!");
				}
...
;(function($) {
	...
	*/
	$.fn.fancyProductDesigner.defaults = {
		...
		* initText: "Initializing Collage",<br />
