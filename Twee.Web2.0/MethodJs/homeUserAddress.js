        ...
        success: function (resu) {
            if (resu == "-1") {
                ...
                */
                ...
                //------------------------
                var pageComeFrom = $("#pageComeFrom").val();
                if (pageComeFrom.length > 0) {
                    window.location.href = "/Product/saleBuy.aspx?id=" + pageComeFrom+"&action=buynow";
                } else {
...
function BindProvinceByName(countryId, proname) {
    $("#selectProvince2").empty();
    $("#selectProvince2").append($("<option/>").text("--select--").attr("value", "-1"));
    $.ajax({
        type: "POST",
        url: "/AjaxPages/userAddressAjax.aspx",
        data: "{action:'area',layer:'1',countryId:'" + countryId + "'}",
        ...
        success: function (obj) {
            ...
            $(obj).each(function (index) {
                ...
                if (val.ProName == proname) {
                    $("#selectProvince2").append($("<option  selected='true' />").text(val.ProName).attr("value", val.ProID));
                }
                else {
