    ...
    $.ajax({
        ...
        success: function (resu) {
           ...
           // $("#bigShowImg").attr("src", picPath + pics[0]);
            ...
            //remove unused slide Add by Long 2016/01/22
            /*
            for (i = pics.length; i < 15; i++) {
                $("#ms_slide_" + (i + 1)).css("display","none");
            }*/
           ...
           // console.log("htrfShare bind click");
            $("#htrfShare").bind("click", function () {
                ...
                if (pageUrl.indexOf("/saleBuy.aspx") != -1) sharePage = "/Product/saleBuy.aspx";
                ...
                SharePrdEx(prdID, baseInfo.name, picFullName, sharePage, share_text, escapeHtml(baseInfo.txtjj));
            });
            $("#btnShareAndEarn").bind("click", function () {
            ...
            //    SharePrdEx(prdID, baseInfo.name, pics[0]);
            //})
            ...
            try {
                ...
                $("#pro-info").html(baseInfo.txtinfo);
            }
...
function LoadVideo(videoPath) {
    var so = new SWFObject("../css/PlayerLite.swf", "CuPlayerV4", "560", "320", "9", "#000000");
    ...
    so.addVariable("bgAlpha", "75");
     so.write("CuPlayer");
  //  change by Long 2016/01/06 so.write("CuPlayer");
    //so.write("divResponsiveVideo");
   // $("#divResponsiveVideo").html(baseInfo.videoEmbed);
...
////立即购买按钮事件  commented by Jack Cao 2016/01/08 this function has never been used.
...
//            if (cartguid != "") {
//                cartguid = "'" + cartguid + "'";
//                window.location.href = "/Product/shopOrder.aspx?cartids=" + escape(cartguid);
//                return;
//            }
//            else {
...
function ChangeSKU(rules_id) {
    ...
    // change image 
    ChangeImage(rules_id);   // added by Jack Cao 2016-01-14 

}


function ChangeImage(sRuleID) {

    if (mySlider.slides.length == 0) return;
    var jsonImageRuleID = eval(imageRuleID);
    if (jsonImageRuleID.length <= 0) return;
    for (var i = 0; i < jsonImageRuleID.length; i++) {
        if (jsonImageRuleID[i].RuleID == sRuleID) {
            mySlider.api.gotoSlide(i);
            return;
        }
    }
}
               ...
               if (colorJson.length > 0) {
                   ...
                   for (var c = 0; c < colorJson.length; c++) {
                       ...
                       if (ruleids[i] == ruleid) {
                           ...
                           //changed by Long 2016/01/06 base on Snow's Request
                           var colorBorder = ";border:solid 2px #ccc;"
                          
                           if (ruleid == selectedRuleid) {
                               $("#proColorArea").attr("selectedruleid", ruleid);

                               colorBorder = ";border:solid 2px #555;"  // selected border color
...
function _ChangeColor(colorID, colorLabelID) {
  ...
  $("[id^='colorLabel-']").css("border", "2px").css("border-color", "#ccc").css("border-style", "solid");
  
  // set select border to red
  $("#" + colorLabelID).css("border", "2px").css("border-color", "#555").css("border-style", "solid");
  $("#proColorArea").attr("selectedRuleid", ruleid);
...
function _PreChangeRule(obj) {
    ...
    if (ruleids.length > 0) {
        ...
        for (var i = 0; i < ruleids.length; i++) {
            if (colorArea != null && colorArea.length > 0) {
                ...
                if (colorJson.length > 0) {
                    ...
                    for (var c = 0; c < colorJson.length; c++) {
                        ...
                        if (ruleids[i] == ruleid) {
                            if (color=="#fff") {
                                colorHtml += "<div ruleid=" + ruleid + " onclick=\"_PreChangeColor(this)\" class=\"selectColor\"><div style=\" background:" + color + ";border:solid 2px #ccc;\"></div></div>";
                            }
                            else {
            ...
            //默认选中第一个颜色
            var firstColor = $("#proColorArea").find("div").eq(0);
            firstColor.css("border", "2px").css("border-color", "#555").css("border-style", "solid");
            $("#proColorArea").attr("selectedRuleid", firstColor.attr("ruleid"));
...
function _PreChangeColor(obj) {
    $("#proColorArea").find("div[class='selectColor']").css("border", "0px");
    $(obj).css("border", "2px").css("border-color", "#555").css("border-style", "solid");
