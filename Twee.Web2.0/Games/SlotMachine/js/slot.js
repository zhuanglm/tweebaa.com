...
var IMAGE_TOP_MARGIN = -30;
var IMAGE_BOTTOM_MARGIN = 30;
var SLOT_SEPARATOR_HEIGHT = 20;
...
function preloadImages( images, callback ) {
	...
	asset.img = new Image();
	asset.img.src = '/images/slot_machine/img/' + asset.id + '.png';
...
function SlotGame() {
    ...
    preloadImages(items, function () {
        ...
        function _fill_canvas(canvas, items) {
            ...
            /*
            var img = document.getElementById("slot_bg");
    ...
    $('#play').click(function (e) {
        ...
        if (userGuid.length < 10) {
            var r = confirm("You must Login before you play, Are you goto Login page now?");
            if (r == true) {
                window.location.href = "/User/login.aspx?op=slot_machine";
            }

        } else {
            var p = TweebaaWebApp2.Games.SlotMachine.index.SlotMachineCheckCredit();


            if (p.value == 1) {
                //update credit
                var ret = TweebaaWebApp2.Games.SlotMachine.index.SlotMachineUpdateCredit();
                if (parseInt(ret.value) > 0) {
                    $("#divMyPoints").html(ret.value);
                    console.log("credit update =" + ret.value);
                    game.restart();
                } else {
                    alert("Sorry, you may play too many times? ret=" + ret.value);
                }
            } else {
                alert("Sorry, you don't have enough Share Points");
            }
        }
    });
...
Game.prototype.restart = function () {
    ...
    for (var i = 0, l = this.items1.length; i < l; i++) {
        values1.push(this.items1[i].id);
    }
    var values2 = [];

    for (var i = 0, l = this.items2.length; i < l; i++) {
        values2.push( this.items2[i].id);
    }
    var values3 = [];

    for (var i = 0, l = this.items3.length; i < l; i++) {
        values3.push(this.items3[i].id);
    }
    // get random results
    this.result1 = parseInt(TweebaaWebApp2.Games.SlotMachine.index.SlotMachineCreatRandomNumberWithIndex(this.items1.length, 1, values1.toString()).value)
    this.result2 = parseInt(TweebaaWebApp2.Games.SlotMachine.index.SlotMachineCreatRandomNumberWithIndex(this.items2.length, 2, values2.toString()).value)
    this.result3 = parseInt(TweebaaWebApp2.Games.SlotMachine.index.SlotMachineCreatRandomNumberWithIndex(this.items3.length, 3, values3.toString()).value)
...
Game.prototype.update = function () {
    ...
    switch (this.state) {
        ...
        case 6: // check results
            ...
            ec = TweebaaWebApp2.Games.SlotMachine.index.SlotMachinCheckResult().value;
            var pp = ec.split(":");
            console.log("ec=" + pp[0]);
            //$('#multiplier').text(ec);

            $('#status').text(BLURB_TBL[pp[0]]);
            $("#divMyPoints").html(pp[1]);
            this.state = 7;
            break;
        case 7: // game ends
            break;
