...
namespace TweebaaWebApp2.Mgr.proManager
{
    ...
    public class proPublicJudgeMgr1 : IHttpHandler
    {
        ...
        private string InitData(HttpContext context)
        {
            ...
            {
                json.AppendFormat(",{0}  \"prdguid\":\"{2}\",\"prdname\":\"{3}\",\"prdcate\":\"{4}\",\"estimateprice\":\"{5}\",\"saleprice\":\"{6}\",\"wnstat\":\"{7}\",\"username\":\"{8}\",\"addtime\":\"{9}\",\"supplyprice\":\"{10}\",\"userid\":\"{11}\",\"ranking\":\"{12}\"  {1}", "{", "}"
...
//]}";
            ...
            Twee.Mgr.PrdMgr mgr = new Twee.Mgr.PrdMgr();
            int totalCount = 0;
            var dt = mgr.MgeGetListReview(proName, proUser, state, cate, startTime, endTime, "edttime desc", startId, endId,out totalCount);
            //if (dt == null || dt.Rows.Count == 0)
            //    return string.Empty;
            StringBuilder json = new StringBuilder();
            foreach (DataRow row in dt.Rows)
            {
                ...
                json.AppendFormat(",{0}  \"prdguid\":\"{2}\",\"prdname\":\"{3}\",\"prdcate\":\"{4}\",\"estimateprice\":\"{5}\",\"saleprice\":\"{6}\",\"count0\":\"{7}\",\"count1\":\"{8}\",\"wnstat\":\"{9}\",\"username\":\"{10}\",\"addtime\":\"{11}\",\"supplyprice\":\"{12}\",\"ranking\":\"{13}\"  {1}", "{", "}"
                    , row["prdguid"]._ObjToString(),
                    row["name"]._ObjToString().Replace(',', '，').Replace("\"", "&quot;"),
                    row["cname"]._ObjToString().Replace(',', '，').Replace("\"", "&quot;"), 
                    Math.Round((decimal)row["estimateprice"],2)._ObjToString(),
        ...
        private string TweeGridData(HttpContext context)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                json.AppendFormat(",{0}  \"prdguid\":\"{2}\",\"prdname\":\"{3}\",\"prdcate\":\"{4}\",\"estimateprice\":\"{5}\",\"saleprice\":\"{6}\",\"count0\":\"{7}\",\"count1\":\"{8}\",\"wnstat\":\"{9}\",\"username\":\"{10}\",\"addtime\":\"{11}\",\"stateremark\":\"{12}\",\"presaleforward\":\"{13}\",\"presaleendtime\":\"{14}\",\"ranking\":\"{15}\"  {1}", "{", "}"
                    , row["prdguid"]._ObjToString(),
                    row["name"]._ObjToString().Replace(',', '，').Replace("\"", "&quot;"),
                    row["cname"]._ObjToString().Replace(',', '，').Replace("\"", "&quot;"),
                    Math.Round((decimal)row["estimateprice"],2)._ObjToString(),
        ...
        private string PreSaleGridData(HttpContext context)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                json.AppendFormat(",{0}  \"prdguid\":\"{2}\",\"prdname\":\"{3}\",\"prdcate\":\"{4}\",\"saleprice\":\"{5}\",\"count\":\"{6}\",\"saletotal\":\"{7}\",\"wnstat\":\"{8}\",\"username\":\"{9}\",\"addtime\":\"{10}\",\"stateremark\":\"{11}\",\"presaleforward\":\"{12}\",\"presaleendtime\":\"{13}\",\"hottip\":\"{14}\",\"ranking\":\"{15}\"  {1}", "{", "}"
                    , row["prdguid"]._ObjToString(),
                    row["name"]._ObjToString().Replace(',', '，').Replace("\"", "&quot;"),
                    row["cname"]._ObjToString().Replace(',', '，').Replace("\"", "&quot;"),
                    row["saleprice"]._ObjToString(),
        ...
        private string UpSaleGridData(HttpContext context)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                json.AppendFormat(",{0}  \"prdguid\":\"{2}\",\"prdname\":\"{3}\",\"prdcate\":\"{4}\",\"saleprice\":\"{5}\",\"count\":\"{6}\",\"saletotal\":\"{7}\",\"wnstat\":\"{8}\",\"username\":\"{9}\",\"addtime\":\"{10}\",\"stateremark\":\"{11}\",\"presaleforward\":\"{12}\",\"ranking\":\"{13}\"  {1}", "{", "}"
                    , row["prdguid"]._ObjToString(),
                    row["name"]._ObjToString().Replace(',', '，').Replace("\"", "&quot;"),
                    row["cname"]._ObjToString().Replace(',', '，').Replace("\"", "&quot;"),
                    row["saleprice"]._ObjToString(),
        ...
        private string SaleingGridData(HttpContext context)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                json.AppendFormat(",{0}  \"prdguid\":\"{2}\",\"prdname\":\"{3}\",\"prdcate\":\"{4}\",\"count\":\"{5}\",\"saletotal\":\"{6}\",\"wnstat\":\"{7}\",\"username\":\"{8}\",\"addtime\":\"{9}\",\"stateremark\":\"{10}\",\"ranking\":\"{11}\"  {1}", "{", "}"
                    , row["prdguid"]._ObjToString(),
                    row["name"]._ObjToString().Replace(',', '，').Replace("\"", "&quot;"),
