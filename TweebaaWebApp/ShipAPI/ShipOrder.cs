...
namespace TweebaaWebApp.ShipAPI
{
    public class ShipOrder
    {
        ...
        public string CreatePackListPDFReport(int iShipOrderID)
        {
            ...
            string sDBServerName = System.Configuration.ConfigurationManager.AppSettings["DBServerName"];
            string sDBName = System.Configuration.ConfigurationManager.AppSettings["DBName"];
            string sDBUserName = System.Configuration.ConfigurationManager.AppSettings["DBUserName"];
            string sDBUserPassword = System.Configuration.ConfigurationManager.AppSettings["DBUserPassword"];
        ...
        private bool SendDropShipperEmail(DropShipperOrder dropShipperOrder)
        {
            try
            {
                ...
                foreach (DropShipperOrderItem item in dropShipperOrder.ItemList)
                {
                    ...
                    sOrderItem.Append("<td style=\"border-top: #eee 1px solid; \">" + item.sDesc + "</td>");
        ...
        private ShipOrderResult SendDropShipperOrder(DropShipperOrder dropShipperOrder)
        {
            ...
            try
            {
                ...
                if (iSuccess == 1)
                {
                    ...
                    sSql.Append(" where headguid in ( select guid from wn_OrderHead where guidno ='" + dropShipperOrder.sOrderHeadGuidNo + "')");
        ...
        private ShipOrderResult SendWarehouseShipOrder(FosdickOrder warehouseOrder)
        {
            ...
            try
            {
                ...
                // get new Ship Order ID
                iShipOrderID = db.DBGetSeq("ShipOrderID");
                result.iShipOrderID = iShipOrderID;

                // insert order info
                StringBuilder sSql = new StringBuilder();
                sSql.Append("insert into wn_ShipOrder(ShipOrder_ID, OrderHead_GuidNo, ShipPartner_ID, ShipOrder_IsSuccess, ShipOrder_Date, ShipOrder_Status, ShipOrder_ReqXML, ShipOrder_ResponseXML, ShipOrder_ErrMsg, ShipOrder_PartnerOrderID, ShipOrder_WarehouseErrorCode, ShipOrder_WarehouseErrorDetail)");
                sSql.Append(" values(");
                sSql.Append(iShipOrderID.ToString() + ",");                                             // ship order id
                ...
                {
                    sSql.Clear();
                    sSql.Append("update wn_OrderBody set ShipOrder_ID=" + iShipOrderID.ToString());
                    sSql.Append(" where headguid in ( select guid from wn_OrderHead where guidno ='" + warehouseOrder.sOrderHeadGuidNo + "')");
                    int iRow = db.DBExecute(sSql.ToString());
        ...
        private ShipOrderResult  SendEOrderOrder(EOrderReq eOrder)
        {
            ...
            try
            {
                ...
                if (iSuccess == 1)
                {
                    sSql.Clear();
                    sSql.Append("update wn_OrderBody set ShipOrder_ID=" + iShipOrderID.ToString());
                    sSql.Append(" where headguid in ( select guid from wn_OrderHead where guidno ='" + eOrder.sOrderHeadGuidNo + "')");
                    int iRow = db.DBExecute(sSql.ToString());
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (iShipPartnerID == (int)(ConfigParamter.ShipPartner.Fosdick))
                {
                    ...
                    if (iShipPartnerID != iPrevShipPartnerID)
                    {
                        ...
                        warehouseOrder.sExternalID = sOrderNo;
                        warehouseOrder.sAdCode = "TEST";
                        ...
                        warehouseOrder.sShipLastName = dr["ShipTo_LastName"].ToString();
