<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="proCheck.aspx.cs" Inherits="TweebaaWebApp.Mgr.proManager.proCheck" %>
        ...
        <script type="text/javascript">
            $(function () {
                ...
                $("#ddlTypeOne").combobox({
                    valueField: 'value',
                    textField: 'text',
                    data: eval(TweebaaWebApp.Mgr.proManager.proCheck.GetFirstCate().value),
                    onSelect: function (e) {
                        var selectedId = e.value;
                        $("#ddlTypeTwo").combobox("clear");
                        $("#ddlTypeThree").combobox("clear");
                        if (selectedId != "-1") {
                            var res = TweebaaWebApp.Mgr.proManager.proCheck.GetNextCate(selectedId).value;
                            if (res != "fail") {
        ...
        <script type="text/javascript">
                    ...
                    columns: [[
                        ...
                        { field: 'addtime', title: 'Submit Time', width: 100 },
                        {
                            field: '_edit', title: 'Operation', width: 250, formatter: function (value, row) {
                                return "<a href='javascript:void(0)' onclick=_view(this) dbkey='" + row.prdguid + "'>Check</a>&nbsp;&nbsp;<a href='javascript:void(0)' dbuserid='"+row.userid+"' dbkey='" + row.prdguid + "' dbproname='" + row.prdname + "' onclick=passSingle(this)>通过初审</a>&nbsp;&nbsp;<a href='javascript:void(0)' dbuserid='"+row.userid+"' dbproname='" + row.prdname + "' dbkey='" + row.prdguid + "' onclick=nopassSingle(this)>拒绝通过</a>&nbsp;&nbsp;<a href='javascript:void(0)' dbkey='" + row.prdguid + "' onclick=proEdit(this)>编辑该产品</a>"
                            }
                        }
                    ]]
            ...
            function passAll() {
                if (confirm("Confirm to approve the selected items?")) {
                    ...
                    if (checkedItems.length == 0) {
                        ...
                        if (proids.length > 0) {
                            var res = TweebaaWebApp.Mgr.proManager.proCheck.PassAll(proids).value;
                            if (res == "success") {
            ...
            function passSingle(obj) {
                if (confirm("Confirm to approve the selected items?")) {
                    ...
                    if (proid.length > 0) {
                        var res = TweebaaWebApp.Mgr.proManager.proCheck.PassSingle(proid,proname,userid).value;
                        if (res == "success") {
                            alert("Approve Success！");
                            _search();
                        }
            ...
            function noPassSave() {
                ...
                if (proid == null || reason == null || proid == "" || $.trim(reason) == "") {
                    ...
                    var res = "";
                    res = TweebaaWebApp.Mgr.proManager.proCheck.RefusePass(proid, proname,userid,reason).value; 
                    if (res == "success") {
