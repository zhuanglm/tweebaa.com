<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="proPreSaleMgr.aspx.cs" Inherits="TweebaaWebApp.Mgr.proManager.proPreSaleMgr" %>
...
<fieldset>
        ...
        <table class="tbtable" style="width:100%;">
            ...
            <tr>
                ...
                <td class="title">Limited Time Status：<asp:HiddenField ID="hidState" runat="server" /></td>
                <td>
   ...
   <div style=" margin:2px;">
        ...
        <div id="preSaleDiv" title="Edit Limited Time Price, Target, Time">
                   <table id="Table1" class="tbtable" style="width:300px;">
                       <tr class="head">
                           <td class="title" style=" width:100px; text-align:right">Limited Time Price：</td>
                           <td><input type="text"  id="txtPrePrice" /></td>
                       </tr>
                       <tr class="head">
                           <td class="title" style=" width:100px; text-align:right">Limited Time Target：</td>
        ...
        <script type="text/javascript">
            $(function () {
                ...
                $("#ddlTypeOne").combobox({
                    valueField: 'value',
                    textField: 'text',
                    data: eval(TweebaaWebApp.Mgr.proManager.proPreSaleMgr.GetFirstCate().value),
                    onSelect: function (e) {
                        var selectedId = e.value;
                        $("#ddlTypeTwo").combobox("clear");
                        $("#ddlTypeThree").combobox("clear");
                        if (selectedId != "-1") {
                            var res = TweebaaWebApp.Mgr.proManager.proPreSaleMgr.GetNextCate(selectedId).value;
                            if (res != "fail") {
                ...
                $("#ddlState").combobox({
                    valueField: 'value',
                    textField: 'text',
                    data: eval("[{value:'-1',text:'--ALL--'},{value:'2',text:'Limited Time'},{value:'7',text:'Limited Time Fail'}]"),
                    onSelect: function (e) {
        ...
        <script type="text/javascript">
            ...
            $(function () {
                var pager = $('#dg').datagrid({
                    ...
                    columns: [[
                        ...
                        { field: 'presaleforward', title: 'Limited Time Target', width: 60, align:'right'},
                        { field: 'presaleendtime', title: 'Limited Time End', width: 80 },
                        ...
                        { field: 'wnstat', title: 'Limited Time Status', width: 80 },
            ...
            function passAll() {
                ...
                if (confirm("Confirm to approve the selection？")) {
                    ...
                    if (checkedItems.length == 0) {
                        ...
                        if (proids.length > 0) {
                            var res = TweebaaWebApp.Mgr.proManager.proPreSaleMgr.PassAll(proids, adminid).value;
                            if (res == "success") {
            ...
            function passSingle() {                
                ...
                var exreason = $("#passExReason").val();
                    if (proid.length > 0) {
                        var res = TweebaaWebApp.Mgr.proManager.proPreSaleMgr.PassSingle(proid, reasonid, exreason, adminid).value;
                        if (res == "success") {
                            alert("Evaluation Success！");
                            _search();
                            $('#passdiv').window('close');
            ...
            function OpenPassDiv(obj)
            {
                ...
                if (isNaN(order) || isNaN(count)) {
                    alert("Have not reach Limited Time target, cannot pass");
                    return;
                }
                if (order > count) {
                    alert("Have not reach Limited Time target, cannot pass");
            ...
            function noPassSave() {
                ...
                if (proid == null || reason == null || proid == "" || $.trim(reason) == "") {
                    ...
                    var res = TweebaaWebApp.Mgr.proManager.proPreSaleMgr.NoPass(proid, reason).value;
                    if (res == "success") {
                        alert("Evaluation Success");
                        $('#winView').window('close');
                        _search();
        ...
        <script type="text/javascript">
            ...
            function FowardSave() {
                ...
                if (proids.length > 0) {
                    ...
                    if (order == "" || !isPInt(order)) {
                        alert('Please enter Limited Time target number (in interger)'); return;
                    }
                    var res = TweebaaWebApp.Mgr.proManager.proPreSaleMgr.SavePreSale(proids, prePrice, order, time).value;
                    if (res == "success") {
                        alert("Edit Success");
                        $('#preSaleDiv').window('close');
                        _search();
            ...
            function proSetHot(obj) {
                ...
                if (proids.length > 0) {
                    var res = TweebaaWebApp.Mgr.proManager.proPreSaleMgr.SetHot(proids).value;
                    if (res == "success") {
                        alert("Set Success");
                        _search();
                    } else {
            ...
            function proCancelHot(obj) {
                ...
                if (proids.length > 0) {
                    var res = TweebaaWebApp.Mgr.proManager.proPreSaleMgr.UnSetHot(proids).value;
                    if (res == "success") {
                        alert("Set Success");
                        _search();
                    } else {
