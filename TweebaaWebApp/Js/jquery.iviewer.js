    ...
    */
    ...
    $.widget("ui.iviewer", $.ui.mouse, {
        ...
        **/
        ...
        fit_height: function (skip_animation) {
            /*
            var aspect_ratio = this.img_object.orig_width() / this.img_object.orig_height();
            var window_ratio = this.options.width / this.options.height;
            var choose_left = (aspect_ratio > window_ratio);
        ...
        fit_width: function (skip_animation) {
            /*
            var aspect_ratio = this.img_object.orig_width() / this.img_object.orig_height();
            var window_ratio = this.options.width / this.options.height;
            var choose_left = (aspect_ratio > window_ratio);
        ...
        **/
        ...
        get_img_src: function () {
            if (!this.img_object.loaded()) {
                console.log(" image not load yet");
                return "";
            }
            return this.img_object._img.attr("src");
        },

        update_css: function (x, y, w, h) {
            if (!this.img_object.loaded()) {
                console.log(" image not load yet");
                return;
            }

            if (x == "0px" && y == "0px" && w == "0px" && h == "0px") {
                ...
                this.img_object._img.css(params);
            }
            console.log("should change css?x=" + x + " y=" + y + " w=" + w + " h=" + h);
        },
        update_src_css: function (x, y, w, h, src_new) {
            if (!this.img_object.loaded()) {
                console.log(" image not load yet");
                return;
            }
        ...
        reset_image: function () {
            ...
            // this.img_object._img.removeAttr('src');
            this.img_object._img.attr('src', "/images/spacer.gif");
            /*
            this.img_object._img = $("<img>")
            //this is needed, because chromium sets them auto otherwise
            .css({ position: "absolute", top :"0px", left: "0px"});
            */
            /*        
            this.img_object.setImageProps(w, h, x, y,
            false, function() {
        ...
        */
        info: function (param, withoutRotation) {
            if (!param) { return; }
            /*
            console.log("param="+param+" 1="+this.img_object.object().css('width')+" 2="+this.img_object.object().css('height')
            +" 3="+this.img_object.object().css('top')+" 4="+this.img_object.object().css('left'));
            */
            switch (param) {
                case 'width':
                    return this.img_object.object().css('width');
                case 'height':
                    return this.img_object.object().css('height');
                case 'left':
                    return this.img_object.object().css('left');
                case 'top':
                    return this.img_object.object().css('top');
                case 'orig_width':
                case 'orig_height':
                    if (withoutRotation) {
